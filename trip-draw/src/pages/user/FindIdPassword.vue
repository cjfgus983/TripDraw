  ...
  'bg-[#9FB3DF] bg-opacity-70': activeTab !== 'findPassword'
        ...
        <div v-if="activeTab === 'findPassword'" class="p-8">
          ...
          <div class="space-y-6">
            ...
            <div v-if="showPasswordVerificationField" class="space-y-2">
              <label
...
// API 기본 주소 설정 (환경변수로 관리하거나, dev에서는 localhost)
...
// 비밀번호 찾기 폼 데이터 (코드 검증 없이 바로 발송)
const findPasswordForm = ref({
  email: "",
  verificationCode: "",       // 인증번호 입력 필드용
});

// 인증번호 필드 표시 여부
const showVerificationField = ref(false);
// 비밀번호 찾기 인증번호 필드 표시 여부
const showPasswordVerificationField = ref(false);
...
async function sendVerificationCode() {
  if (!findIdForm.value.name || !findIdForm.value.email) {
    ...
    return showResultModal.value = true;
  }
  try {
...
async function sendPasswordVerificationCode() {
  if (!findPasswordForm.value.email) {
    resultTitle.value = "입력 오류";
    resultMessage.value = "이메일을 입력해주세요.";
    showResultModal.value = true;
    return;
  }
  try {
    await axios.post("/api/users/find-password", {
      email: findPasswordForm.value.email,
    });
    showPasswordVerificationField.value = true;
    resultTitle.value = "임시 비밀번호 발송";
    resultMessage.value = "해당 이메일로 임시 비밀번호가 발송되었습니다.";
  } catch (e) {
    resultTitle.value = "요청 실패";
    const error: any = e;
    resultMessage.value = error.response?.data?.message || "임시 비밀번호 발송에 실패했습니다.";
  } finally {
    showResultModal.value = true;
  }
}

const findPassword = sendPasswordVerificationCode;
