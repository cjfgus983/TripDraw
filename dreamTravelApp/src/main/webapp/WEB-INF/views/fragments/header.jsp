...
<div class="container">
    ...
    <header class="d-flex justify-content-center my-5 align-items-center">
        <h1 class='text-center'>Dream Travel</h1>

    </header>
    <div class="d-flex justify-content-end">
        <a href="${root }/" class="mx-3">홈으로</a>
        |
        <a href="${root }/auth/user-list?currentPage=1" class="mx-3">멤버목록</a>
        |

        <c:if test="${empty loginUser }">
            <a href="${root }/login" class="mx-3">로그인</a> |
            <a href="${root }/user/user-signup" class="mx-3">회원가입</a>
        </c:if>
        <c:if test="${!empty loginUser }">
            <a href="${root }/auth/member-detail?email=${loginUser.email}" class="mx-3">${loginUser.name }</a> |
            <a href="${root }/logout" class="mx-3">로그아웃</a>
        </c:if>
    </div>
    <hr />
</div>
