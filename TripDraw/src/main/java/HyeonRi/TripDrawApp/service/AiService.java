...
//            double longitude = node.path("lng").asDouble();
    ...
    private String fetchPhotoReference(String place) throws IOException {
        ...
        System.out.println("▶ [fetchPhotoReference] URL = " + tsUrl);

        JsonNode tsRoot = restTemplate.getForObject(tsUrl, JsonNode.class);
        JsonNode results = tsRoot.path("results");

        System.out.println("▶ [fetchPhotoReference] 결과 수 = " + results.size());
        ...
        if (results.isArray()) {
            for (JsonNode result : results) {
                ...
                if (photos.isArray() && photos.size() > 0) {
                    ...
                    System.out.println("▶ [fetchPhotoReference] photo_reference = " + photoRef);
                    return photoRef;
                }
            }
        }

        System.out.println("▶ [fetchPhotoReference] 유효한 photo_reference를 찾지 못함");
        return null;
    }

    private String buildPhotoUrl(String photoReference) {
        if (photoReference == null || photoReference.isEmpty()) {
            System.out.println("▶ [buildPhotoUrl] photoReference 없음, fallback 반환");
        ...
        System.out.println("▶ [buildPhotoUrl] 생성된 URL = " + url);
        return url;
    }
    ...
    public List<PlaceWithLatLng> recommendPlacesByGpt(String imageUrl) throws IOException {
        ...
        for (PlaceWithLatLng place : places) {
            ...
            String searchQuery = (name).trim();  // → "모니터 포인트 등대 미국" 식

            System.out.println("▶ [recommendPlacesByGpt] 검색 쿼리 = " + searchQuery);

            // 3) 장소 설명 한 문장 요청
            String shortDesc = fetchDescription(name);

            // 4) photo_reference 검색
            String photoRef = fetchPhotoReference(searchQuery); // 변경된 쿼리 사용
            String photoUrl = buildPhotoUrl(photoRef);          // fallback 포함
